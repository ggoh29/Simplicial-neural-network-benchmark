...
if __name__ == "__main__":
    train_data = SimplicialComplexDataset('./data', dataset, superpixel_size, RAGBasedEdgeFlow, complex_size=0, train=True)
    train_dataset = DataLoader(train_data, batch_size=batch_size, collate_fn=train_data.batch, num_workers=4, shuffle=True)
    test_data = SimplicialComplexDataset('./data', dataset, superpixel_size, RAGBasedEdgeFlow,complex_size=0, train=False)
